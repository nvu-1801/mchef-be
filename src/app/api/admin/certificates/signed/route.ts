...
async function extractRaw(req: Request): Promise<ExtractResult | null> {
...
async function handleSign(req: Request) {
  ...
  if (raw.mode === "id") {
    ...
    const cert = rows?.[0] as CertificateRow | undefined;
    ...
    const signedData = signed as SignedUrlResponse | null;
