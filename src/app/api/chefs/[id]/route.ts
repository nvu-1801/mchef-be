...
// Định nghĩa một kiểu (type) cho rating để sửa lỗi 'any'
interface ChefRating {
  id: number;
  rater_id: string;
  stars: number;
  comment: string | null;
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const avgRating =
      totalRatings > 0
        ? // SỬA LỖI 'any': Thêm kiểu cho 'sum' và 'r'
          (ratings as ChefRating[]).reduce(
            (sum: number, r: ChefRating) => sum + r.stars,
            0
          ) / totalRatings
        : null;
    ...
    // SỬA LỖI 'any': Thêm kiểu cho 'r'
    const ratingsWithUser = (ratings as ChefRating[] | null)?.map(
      (r: ChefRating) => ({
        id: r.id,
        raterId: r.rater_id,
        stars: r.stars,
